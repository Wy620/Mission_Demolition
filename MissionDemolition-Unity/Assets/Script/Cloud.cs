...
public class Cloud : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            Vector3 scale = Vector3.one; // Randomly assign scale
            ...
            scale.y *= 1 - (Mathf.Abs(offset.x) / sphereOffsetScale.x); // Adjust y scale by x distance from core
    ...
    void Restart()
    {
        foreach (GameObject sp in spheres) // clear out old spheres
        {
            Destroy(sp);
        } // end foreach
        Start();
